...
def run(folds=5, model="resnet_18", metric="ACCURACY"):
    ...
    for fold in range(folds):
        ...
        trn_img = [os.path.join(config.main.TRAIN_PATH, i + config.main.IMAGE_EXT) for i in trn_img]
        trn_labels = df_train[config.main.TARGET_VAR].values

        valid_img = df_valid[config.main.IMAGE_ID].values.tolist()
        valid_img = [os.path.join(config.main.TRAIN_PATH, i + config.main.IMAGE_EXT) for i in valid_img]
